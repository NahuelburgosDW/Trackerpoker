...
export function ProfitChart({ data, height = 320 }: Props) {
  ...
  const { points, min, max, areaPath, linePath, len, yTicks } = useMemo(() => {
    if (data.length === 0) return { points: [], min: 0, max: 0, areaPath: '', linePath: '', len: 0, yTicks: [] };
  ...
  if (data.length === 0) {
    return <div className="flex items-center justify-center text-ink-300 text-sm" style={{ height }}>Sin datos para este período</div>;
  }

  const last = points[points.length - 1];
  const positive = last.value >= 0;
  const stroke = positive ? '#2BE5A0' : '#FF5A5F';
  const fill = positive ? `url(#${id}-pos)` : `url(#${id}-neg)`;
